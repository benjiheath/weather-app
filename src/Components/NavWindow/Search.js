  ...
  return (
    <div className="summary search-window">
      <div className="close-wrapper row-80">
        <CgClose className="close-icon" onClick={() => setShowSearch(false)} />
      </div>
      ...
      <div className="search-results-wrapper row-80">
        ...
        {tooManyResults && <p>Too many results - Please search something more specific</p>}
        {isNullResults && <p>No locations match your query.</p>}
        {fieldIsEmpty && <p>Please enter a search term</p>}
        {searchResults &&
          searchResults.map((result, idx) => (
            <SearchResult key={idx} name={result.title} onClick={() => setNewLocation(result.woeid)} />
          ))}
      </div>
    </div>
  );
